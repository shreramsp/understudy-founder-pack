...
- **The unanswered question is not the skill library, it is the raw capture.** Sonia's actual question — *what leaves my environment?* — is about admin-plane session recordings, which must reach somewhere to be clustered. **The pack does not yet state where.** If clustering can run inside the client boundary, or on redacted derivatives that never leave it, the review shortens dramatically and the objection largely dissolves; if it cannot, every environment carries a full data-transfer review. **This is an architecture decision with a direct revenue consequence, and it is owed to `tech/architecture/`.**
...
**Recommended next 3 moves:** (1) write the tenant-isolation document, the RPA differentiation write-up and the pilot agreement before the first pilot conversation — all three gate early steps and none requires product; (2) decide whether clustering can run inside the client boundary, because that single architecture choice sets the length of every security review and therefore the expansion rate the whole model depends on; (3) start the SOC 2 clock early — it gates both the enterprise-adjacent clients and the marketplace channel, and it cannot be compressed later.
