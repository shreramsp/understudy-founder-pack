...
| D6 | **In-boundary clustering is an architecture constraint, not a preference.** Raw recordings never leave the client boundary; only the abstracted variance envelope crosses | 2026-09-04 | Resolves the open question raised in `product/PRD.md` §6 and `strategy/sales_roadmap.md`. Binding on `tech/architecture/`: distillation runs inside the perimeter, and no design may propose shipping raw admin-plane capture out. Shortens the recurring per-environment security review, and makes the compounding half of the library defensible by construction rather than by policy |
